...
class PCHA_KRACH(object):
    ...
    def __init__(self, schedule, matrix = False):
        ...
        for team in self.team_records.keys():
            ...
            self.win_percent[team] = float(wins) / float(total)
            self.win_percent[team]
    ...
    def RPI_ranking(self, output = 'none'):
        ...
        for team in self.records.keys():
            ...
            Vi = float(self.team_records[team]['wins'])
            Li = float(self.team_records[team]['losses'])
            Ni = float(Vi + Li + self.team_records[team]['ties/shootout'])            
            
            second_term = 0.0 # ∑j(Nij/Ni)*(Vj-Vji)/(Nj-Nji) 
            third_term = 0.0 # ∑j(Nij/Ni)*[∑k(Njk/Nj)*(Vk-Vkj)/(Nk-Nkj)] 

            for opponent in self.records[team].keys():
                # Opponent victories, victories over this team
                Vj = float(self.team_records[opponent]['wins'])
                Vji = float(self.records[opponent][team]['wins'])
                # Total games against this opponent
                Nijw = float(self.records[team][opponent]['wins'])
                Nijl = float(self.records[team][opponent]['losses'])
                Nijt = float(self.records[team][opponent]['ties/shootout'])
                Nij = float(Nijw + Nijl + Nijt) # == N_ji
                # Total games of this opponent 
                Lj = float(self.team_records[opponent]['losses'])
                Nj = float(Vj + Lj + self.team_records[opponent]['ties/shootout'])
                ...
                for opp_of_opp in self.records[opponent].keys():
                    ...
                    Njkw = float(self.records[opponent][opp_of_opp]['wins'])
                    Njkl = float(self.records[opponent][opp_of_opp]['losses'])
                    Njkt = float(self.records[opponent][opp_of_opp]['ties/shootout'])
                    Njk = float(Njkw + Njkl + Njkt) # Equal to Nkj
                    # Victories of opponent's other opponent
                    Vk = float(self.team_records[opp_of_opp]['wins'])
                    Vkj = float(self.records[opp_of_opp][opponent]['wins'])
                    # Total games of other opponent
                    Lk = float(self.team_records[opp_of_opp]['losses'])
                    Nk = float(Vk + Lk + self.team_records[opp_of_opp]['ties/shootout'])
